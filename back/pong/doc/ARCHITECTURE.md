...
- `Room`: `id`, `players`, `linkToken`, `settings`, `state`, `createdAt`, `status (waiting|running|ended)`.
- `Player`: `id`, `name` (optionnel), `team (left|right|neutral)`, `color`, `isLeader`, `socket`, `lastInput`, `isBot`.
- `Settings`: `playersPerTeam (1-5)`, `paddleSize`, `paddleSpeed`, `ballSpeedInit`, `aiStrength`, `bestof(5-21)`.
- `State`: `score {left,right}`, `ball {x(0.0-400.0),y(0.0-300.0),vx(0.0-1.0),vy(0.0-1.0),speed(ballSpeedInit-ballSpeedInit*5)}`, `paddles {playerId, x, y, vy}`, `tick`.
